...
class DataGenerator(DataGenerator):
    ...
    def __iter__(self, random=False):
        ...
        for is_end, d in self.sample(random):
            ...
            labels = np.zeros((len(token_ids), 1))
            for start, end, label in d[1:]:
                if start in start_mapping and end in end_mapping:
                    start = start_mapping[start]
                    end = end_mapping[end]
                    labels[start, 0] = self.categories.index(label) * 2 + 1
                    labels[start + 1:end + 1, 0] = self.categories.index(label) * 2 + 2
            ...
            if len(batch_token_ids) == self.batch_size or is_end:
                ...
                batch_labels = sequence_padding(batch_labels, value=-1)
